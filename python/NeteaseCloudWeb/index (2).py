...
def run(url):
    ...
    try:
        ...
        while x1 < index_hot_num:
            ...
            index_hot = driver.find_elements_by_xpath(
                "//div[@id='discover-module']//ul[@class='m-cvrlst f-cb']/li/div[@class='u-cover u-cover-1']")
            if util.check_content('<i class="u-icn u-icn-53"></i>', index_hot) is not None:
                x1 += 1
                continue
            ...
            sql1 = "INSERT INTO `music`.`album` (`aId`, `aImgSrc`, `aTitle`, `aCollectNum`, `aFowardingNum`, `aCommentNum`, `aTags`, `aIntroduce`, `aPlayNum`,`aaId`) " \
                   "VALUES (NULL,'" + play_list_cover + "', '" + play_list_name + "', '" + str(
                collect_num) + "', '" \
                               "" + str(forwarding_num) + "', '" + str(
                comment_num) + "', '" + label + "', '" + introduce + "', NULL,'" + str(playlist_id) + "');"
            try:
                cursor.execute(sql1)
                db.commit()
            ...
            while x4 < total_songs_num:
                ...
                x4 += 1
                song_src = "http://music.163.com/song/media/outer/url?id=" + str(song_id) + ".mp3"
                sql2 = "INSERT INTO `music`.`musicinfo` (`mId`, `musicSrc`, `mTitle`, `mAlbum`, `mSinger`, `mLyrics`, `mDuring`, `mCompose`, `mPlaylistId`,`mmId`) VALUES " \
                       "(NULL, '" + song_src + "', '" + song_name + "', '" + album_name + "', '" + singer_name + "', NULL, '" + during + "', NULL, '" + str(
            ...
            while x2 < total_songs_num:
                ...
                time.sleep(5)
                hot_comments = driver.find_elements_by_xpath("//div[@class='cmmts j-flag']/div")
                ...
                while x3 < total_comment_num:
                    ...
                    x3 += 1
                    song_id = songs_id[x2]
                    sql3 = "INSERT INTO `music`.`user` (`uId`, `uName`, `password`, `status`, `headImg`,`uuId`) VALUES (NULL, '" + user_name + "', NULL, NULL, '" + user_head + "','" + str(
                        user_id) + "');"
